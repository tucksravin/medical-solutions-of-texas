<script lang='ts'>
  ...
  let { accordianProps = [{label:"Is this thing on?", content:LOREM},{label:"Is this thing on?"}] } = $props();
  ...
  <div class="w-full flex flex-col border-dark border-b-2 cursor-pointer">
    {#each accordianProps as accordian, i}
      <button class="w-full border-t-2 border-dark cursor-pointer flex flex-col xl:flex-row relative" onclick={() => activeAccordians[i] = !activeAccordians[i]}>
        ...
        {/if}
        <i class="absolute right-8 top-10 fa-sharp fa-thin fa-plus fa-2xl scale-150 text-dark transition-transform duration-300 ease-in-out opacity-80 hover:opacity-100 origin-center {activeAccordians[i] ? 'rotate-45' : ''}"></i>
      </button>
    {/each}
  </div>
