<script lang="ts">
	...
	interface Props {
		accordianProps?: AccordianItem[];
	}

	let {
		accordianProps = [{ label: "Is this thing on?", content: LOREM }, { label: "Is this thing on?" }]
	}: Props = $props();

	let activeAccordians: boolean[] = $state(accordianProps.map(() => false));
</script>

<div class="w-full flex flex-col border-dark border-b-2 cursor-pointer">
	{#each accordianProps as accordian, i}
		<button
			class="w-full border-t-2 border-dark cursor-pointer flex flex-col xl:flex-row relative"
		...
		>
			...
			{#if activeAccordians[i]}
				<div class="pr-16 w-full" transition:slide={{ duration: 500 }}>
					<p
						class="text-left pb-8 pr-8 pt-4 text-[16px] whitespace-pre-wrap leading-relaxed md:text-[24px] xl:text-[20px] transition-opacity {activeAccordians[
							i
						]
							? 'opacity-100'
							: 'opacity-0'}"
						transition:fade>{accordian.content || LOREM}</p
					>
				</div>
			{/if}
			<Plus
				class="absolute right-8 top-10 text-dark transition-transform duration-300 ease-in-out opacity-80 hover:opacity-100 origin-center {activeAccordians[
					i
				]
					? 'rotate-45'
					: ''}"
